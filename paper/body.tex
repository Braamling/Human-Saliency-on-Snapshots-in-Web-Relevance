...
\section{Results}
In this section, we show the results of the benchmark experiments and visual extraction methods discussed in section \ref{sec:experiments}. 

The results of the benchmark experiments are shown in Table \ref{tab:11vs46}. Each model was trained by optimizing the $P@5$ using the validation set. The results show that using the features from \datasetname compared to the feature from LETOR has a minor impact on performance.  

Table \ref{tab:visresults} shows the results using the different visual ranking methods and the ViP baseline. A two tailed paired t-test was used for significance. 
