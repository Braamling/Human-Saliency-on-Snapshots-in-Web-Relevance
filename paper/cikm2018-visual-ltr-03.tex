...
\todo{How many queries do we have there? How many judged documents per query?}
...
\todo{Then how do we compute features and what features exactly?}
...
\item a set of files with the corresponding content features (Section~\ref{sec:contentfeature}).
\end{inparaenum}
...
These fold-partitions are then divided in five folds containing three \todo{equal?} fold-partitions for training and the remaining two for validation and testing.
\todo{Why do we need to additionally split each fold? So we have five fold split into five fold each?}
