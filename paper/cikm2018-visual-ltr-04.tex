...
Since the Wayback Machine does not contain an archived or working version of each document in the ClueWeb12 collection, a filtering process is introduced to maximize the quality of each snapshot. Using the following criteria, a snapshot is selected for each available document. 
...
\end{enumerate}
%
As a result, most of the 28,906 judged documents have a corresponding snapshot from either the Wayback Machine or ClueWeb12 rendering service; only 265 documents did not pass the filtering and were discarded.
Table~\ref{tab:countsources}, row one, summarizes the results of the \datasetname{} dataset~acquisition process. Additionally, the table also shows the distribution of judgment scores among the snapshots that were discarded, taken from the Wayback Machine and taken from the ClueWeb12 rendering service. 
