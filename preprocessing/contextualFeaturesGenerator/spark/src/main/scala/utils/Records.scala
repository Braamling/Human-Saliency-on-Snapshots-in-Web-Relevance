...
object Records {
  def getWarcRecord(in: WarcRecord): warcSparkRecord = {
    val doc: Document = Jsoup.parse(IOUtils.toString(in.getPayload().getInputStreamComplete()))
    warcSparkRecord(in.header.warcTargetUriStr, in.getHeader("WARC-TREC-ID").value, doc.text(), doc.title())
//    warcSparkRecord(in.header.warcTargetUriStr, in.getHeader("WARC-TREC-ID").value, IOUtils.toString(in.getPayload().getInputStreamComplete()))
  }
